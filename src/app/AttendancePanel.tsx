...
export function AttendancePanel({ classes, students, classId, setClassId, permissions, scopeQuery, onError }: Props) {
  ...
  const classStudents = useMemo(
    () => students.filter(student => student.enrollments.some(enrollment => enrollment.classId === classId)),
    [students, classId]
  );
  ...
  const makeupCandidates = useMemo(() => {
    ...
    return students.filter(student => {
      ...
      const hasOtherActiveClass = student.enrollments.some(enrollment => enrollment.classId !== classId);
      const matches = !term || `${student.code} ${student.name} ${student.parentPhone ?? ''}`.toLowerCase().includes(term);
      return !inCurrent && hasOtherActiveClass && matches;
    });
  }, [students, classId, makeupSearch]);

  const loadSessionDetail = useCallback(async (sessionId: string) => {
    const detail = await apiFetch<{ session: Session }>(`/attendance/sessions/${sessionId}`);
  ...
  const loadSessions = useCallback(async (preferredId?: string) => {
    ...
    try {
      const response = await apiFetch<{ sessions: Session[] }>(`/attendance/sessions?${scopeQuery}${scopeQuery ? '&' : ''}classId=${encodeURIComponent(classId)}`);
      setSessions(response.sessions);
      const target = response.sessions.find(item => item.id === preferredId)
        ?? response.sessions.find(item => item.id === session?.id)
        ?? response.sessions[0];
      if (target) await loadSessionDetail(target.id);
      else setSession(null);
    } catch (error) {
      onError(error instanceof Error ? error.message : 'Không tải được danh sách buổi tập.');
    }
  }, [classId, scopeQuery, session?.id, loadSessionDetail, onError]);
  ...
  const createToday = async () => {
    ...
    setBusy(true);
    try {
      const response = await apiFetch<{ session: Session }>('/attendance/sessions', {
        method: 'POST', body: JSON.stringify({ classId, sessionDate: todayIso(), title: 'Buổi tập' })
      });
  ...
  const markNormal = async (student: StudentItem, status: AttendanceStatus) => {
    ...
    try {
      ...
      if (error instanceof ApiError && error.status === 409 && error.payload?.error === 'ATTENDANCE_CONFLICT') {
        await loadSessionDetail(session.id);
        onError('Dữ liệu vừa được HLV khác cập nhật. Hệ thống đã tải bản mới, hãy thao tác lại.');
      } else apiError(error);
    } finally { setBusy(false); }
  };
  ...
  const markMakeup = async (student: StudentItem) => {
    ...
    try {
      await apiFetch(`/attendance/sessions/${session.id}/records`, {
        method: 'PUT',
        body: JSON.stringify({ studentId: student.id, status: 'PRESENT', type: 'MAKEUP', registeredClassId: origin.classId })
      });
      setMakeupOpen(false);
      setMakeupSearch('');
      await loadSessionDetail(session.id);
  ...
  const markAllPresent = async () => {
    ...
    const records = classStudents.map(student => ({
      ...
      type: 'NORMAL',
      registeredClassId: classId,
      expectedUpdatedAt: recordMap.get(student.id)?.updatedAt
    }));
    setBusy(true);
    try {
  ...
  const finalize = async () => {
    ...
    try {
      await apiFetch(`/attendance/sessions/${session.id}/finalize`, { method: 'POST' });
      await loadSessionDetail(session.id);
    } catch (error) { apiError(error); }
  };
  ...
  const exportAttendance = () => {
    ...
    const all = new Map<string, StudentItem>(classStudents.map(student => [student.id, student]));
    for (const record of session.records ?? []) if (record.student) all.set(record.studentId, record.student);
    [...all.values()].sort((a, b) => a.name.localeCompare(b.name, 'vi')).forEach((student, index) => {
      const record = recordMap.get(student.id);
      rows.push([
        index + 1,
        student.code,
        student.name,
        record?.registeredClass?.name ?? student.enrollments.find(item => item.classId === classId)?.class.name ?? '',
  ...
  const allRows = useMemo(() => {
    const map = new Map<string, StudentItem>(classStudents.map(student => [student.id, student]));
    for (const record of session?.records ?? []) if (record.student) map.set(record.studentId, record.student);
  ...
  return <section>
    <div className="bg-white rounded-2xl border border-slate-200 p-3 mb-3 flex flex-wrap gap-2 items-center">
      <select value={classId} onChange={event => setClassId(event.target.value)} className="flex-1 min-w-44 border rounded-xl px-3 py-2 text-sm">
        <option value="">Chọn lớp</option>{classes.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}
      </select>
      <select value={session?.id ?? ''} onChange={event => loadSessionDetail(event.target.value)} className="flex-1 min-w-40 border rounded-xl px-3 py-2 text-sm">
        <option value="">Chọn ngày</option>{sessions.map(item => <option key={item.id} value={item.id}>{dateLabel(item.sessionDate)} {item.isFinalized ? '🔒' : ''}</option>)}
      </select>
    ...
    <div className="space-y-2">{allRows.map((student, index) => {
      ...
      return <div key={student.id} className={`bg-white rounded-2xl border p-3 flex items-center gap-3 ${record?.type === 'MAKEUP' ? 'border-cyan-300' : ''}`}>
        ...
        {permissions.canTakeAttendance && session && !session.isFinalized && student.enrollments.some(item => item.classId === classId) && <div className="flex gap-1">
          <button aria-label={`Có mặt ${student.name}`} onClick={() => markNormal(student, 'PRESENT')} className={`p-2 rounded-xl ${record?.status === 'PRESENT' ? 'bg-emerald-600 text-white' : 'bg-emerald-50 text-emerald-700'}`}><Check className="w-5 h-5"/></button>
          <button aria-label={`Vắng ${student.name}`} onClick={() => markNormal(student, 'ABSENT')} className={`p-2 rounded-xl ${record?.status === 'ABSENT' ? 'bg-rose-600 text-white' : 'bg-rose-50 text-rose-700'}`}><X className="w-5 h-5"/></button>
        </div>}
      </div>;
    })}</div>
    ...
    {makeupOpen && <div className="fixed inset-0 z-50 bg-black/50 flex items-end sm:items-center justify-center p-3" onClick={() => setMakeupOpen(false)}>
      <div className="bg-white rounded-3xl w-full max-w-lg max-h-[80vh] overflow-hidden" onClick={event => event.stopPropagation()}>
        <div className="p-4 border-b flex items-center gap-2"><UserPlus className="text-cyan-600"/><div className="flex-1"><div className="font-black">Chọn võ sinh học bù</div><div className="text-xs text-slate-500">Chỉ hiển thị võ sinh thuộc lớp khác trong cùng võ đường</div></div><button onClick={() => setMakeupOpen(false)}><X/></button></div>
        ...
        <div className="p-3 overflow-auto max-h-[55vh] space-y-2">{makeupCandidates.map(student => {
          const origin = student.enrollments.find(item => item.classId !== classId);
          return <button key={student.id} onClick={() => markMakeup(student)} className="w-full text-left border rounded-xl p-3 hover:bg-cyan-50"><div className="font-bold">{student.name}</div><div className="text-xs text-slate-500">{student.code} · Lớp gốc: {origin?.class.name ?? 'Không rõ'}</div></button>;
        })}{!makeupCandidates.length && <Empty text="Không tìm thấy võ sinh phù hợp"/>}</div>
      </div>
    </div>}
  </section>;
}
...
function Empty({ text }: { text: string }) {
  return <div className="bg-white border border-dashed rounded-2xl py-10 text-center text-slate-400 text-sm">{text}</div>;
}
