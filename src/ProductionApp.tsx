...
export default function ProductionApp() {
  ...
  const refreshMe = useCallback(async (initial = false) => {
    try {
      ...
      if (caught instanceof ApiError && caught.status === 401) {
        ...
        setError(caught instanceof Error ? caught.message : 'Không thể cập nhật phiên đăng nhập.');
      }
      return false;
    } finally {
  ...
  if (checking) {
    return <div className="min-h-screen bg-slate-950 text-white grid place-items-center"><RefreshCw className="w-6 h-6 animate-spin"/></div>;
  }
  ...
  // If a permission is revoked while this device is open, move away from the now-hidden tab.
  if (!visibleTabs.some(item => item.id === tab)) setTab('attendance');

  const dojoName = user.role === 'SUPER_ADMIN'
    ? selectedDojo?.name || 'Chọn võ đường'
    : user.dojo?.name || 'Võ đường';
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-900 pb-20">
    <header className="sticky top-0 z-30 bg-slate-950 text-white shadow-lg">
      <div className="max-w-6xl mx-auto px-3 py-3 flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="font-black truncate">{dojoName}</div>
          <div className="text-[11px] text-slate-400 truncate">{user.fullName} · {user.role}</div>
        </div>
        {user.role === 'SUPER_ADMIN' && <select value={dojoId} onChange={event => setDojoId(event.target.value)} className="max-w-44 bg-slate-800 border border-slate-700 rounded-lg p-2 text-xs">
          <option value="">Chọn võ đường</option>{dojos.map(dojo => <option key={dojo.id} value={dojo.id}>{dojo.name}</option>)}
        </select>}
    ...
    <main className="max-w-6xl mx-auto px-3 py-3">
      ...
      <div className="flex gap-2 overflow-x-auto mb-3">{visibleTabs.map(item => {
        const Icon = item.icon;
        return <button key={item.id} onClick={() => setTab(item.id)} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold whitespace-nowrap ${tab === item.id ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-200'}`}><Icon className="w-4 h-4"/>{item.label}</button>;
      })}</div>
      ...
      {!scope && user.role === 'SUPER_ADMIN'
        ...
        : <>
          {tab === 'attendance' && <AttendancePanel classes={classes} students={students} classId={classId} setClassId={setClassId} permissions={user.permissions} scopeQuery={scopeQuery} onError={setError}/>} 
          {tab === 'students' && <StudentsPanel students={students} classes={classes} activeClassId={classId} scope={scope} isSuperAdmin={user.role === 'SUPER_ADMIN'} permissions={user.permissions} onReload={loadCore} onError={setError}/>} 
          {tab === 'tuition' && user.permissions.canViewTuition && <TuitionPanel students={students} permissions={user.permissions} scopeQuery={scopeQuery} onError={setError}/>} 
          {tab === 'admin' && canAdmin && <AdminPanel scope={scope} scopeQuery={scopeQuery} currentUserId={user.id} onError={setError}/>} 
        </>}
    </main>

    <nav className="fixed bottom-0 inset-x-0 bg-white border-t z-30">
      <div className="max-w-6xl mx-auto grid" style={{ gridTemplateColumns: `repeat(${visibleTabs.length}, minmax(0, 1fr))` }}>
        {visibleTabs.map(item => { const Icon = item.icon; return <button key={item.id} onClick={() => setTab(item.id)} className={`py-2 flex flex-col items-center text-[10px] font-bold ${tab === item.id ? 'text-indigo-600' : 'text-slate-500'}`}><Icon className="w-5 h-5 mb-0.5"/>{item.label}</button>; })}
      </div>
    </nav>
  </div>;
}
